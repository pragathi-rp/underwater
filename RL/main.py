...
class Agent:
    ...
    def init_model(self, model_path=None):
        ...
        self.queue = tf.queue.RandomShuffleQueue(self.memory_size, 1000, [tf.float32, tf.float32, tf.float32, tf.int64, tf.int64], [[self.feature_length], [self.feature_length], 1, 1, 1])
    ...
    def predict(self, state, is_training=True, use_target_q=True):
        if is_training:
            if self.step<10000:
                ep = 1.0
    ...
    def test(self, is_batch_test=False, use_target_q=True, in_order=False, idx=-1, save_raw=False):
        if is_batch_test:
